 ...
 */
class TopOSScz_CPost_Export_OrderController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('toposscz_cpost/export_order')
            ->_addContent($this->getLayout()->createBlock('toposscz_cpost/export_order'))
            ->renderLayout();
    }
    ...
    public function exportAction()
    {
        ...
        if (!empty($orderIds)) {
        	...
        	if ($invoices->getSize() > 0) {
                	foreach ($invoices as $invoice) {
                    		$allInvoices .= $invoice->getIncrementId().',';
                    		$lastInvoice = $invoice->getIncrementId();
                	}
                	$allInvoices = substr($allInvoices,0,-1);
        	}
		...
		if ($order->getWeight() <= 2) {
			...
			if ($cod == 0)
                ...
                /* Invoice id */
                $content = $this->_addFieldToCsv($content, $delimiter, $lastInvoice);
                $content .= $separator;
